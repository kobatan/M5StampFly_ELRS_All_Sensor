...
class BMI270 {
    ...
    private:
                ...
                const gyroOdr_e gyroOdr)
        {
            ...
            m_config[1].cfg.gyr.noise_perf =  BMI2_PERF_OPT_MODE;	//BMI2_POWER_OPT_MODE;
        ...
        {
      			//static constexpr char* ERROR_STRINGS[35] = {					
      			static const char* ERROR_STRINGS[35] = {
                "NULL_PTR",
                "COM_FAIL",
                "DEV_NOT_FOUND",
                "OUT_OF_RANGE",
            ...
            while (rslt) {
                USBSerial.print(funname);
                USBSerial.print(" failed with code ");
                USBSerial.print(rslt);
                USBSerial.print(": ");
                USBSerial.println(ERROR_STRINGS[-rslt - 1]);
                delay(500);
            }
        }

        static int8_t spi_read( const uint8_t reg, uint8_t * data, const uint32_t count, void * intf_ptr)
        {
        ...
        static int8_t spi_write( uint8_t reg, const uint8_t *data, uint32_t count, void *intf_ptr)
        {
            busData_t * busData = (busData_t *)intf_ptr;
            SPIClass * spi = busData->spi;
            const uint8_t csPin = busData->csPin;
